...
it('should get the files by path', async () => {
  ...
  await fileRepository.save(new FileInfo('notes-1.txt', '/documents'));
  await fileRepository.save(new FileInfo('notes-2.txt', '/documents'));
  await fileRepository.save(new FileInfo('notes-3.txt', '/'));
  await fileRepository.save(new FileInfo('notes-4.txt', '/images'));
